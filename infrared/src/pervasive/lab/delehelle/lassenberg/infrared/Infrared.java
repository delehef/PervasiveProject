...
public class Infrared extends javax.microedition.midlet.MIDlet
{
    ...
    /**
     * Take an IR message specified by a byte array and decode it using the SONY format. Typically passed the byte array
     * returned by readIR().
     *
     * @param val a byte array specifying pulse durations in 64 microsecond time periods
     * @return the decoded message value or SONY_BAD_FORMAT (= -2) if the message was not formatted properly
     */
    ...
    public void writeSONY(long cmd)
    {
        ...
        for (int i = 0; i < 3; i++)
        {   // need to repeat twice w/ 45msec delay between sends
            ir.writeIR(msg);
            Utils.sleep(45);
        }
    }
